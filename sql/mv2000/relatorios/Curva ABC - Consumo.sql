...
Select 
	...
	From 
		...
		Select 
			DECODE 
			( : P_PROD_MESTRE , 'S' , PRODUTO . CD_PRODUTO_TEM , PRODUTO . CD_PRODUTO ) Cd_Produto 
			, Trunc 
					( 
						( 
							Sum 
			...
			, Dbamv . Empresa_produto 
				Where Produto.cd_produto = Empresa_produto.cd_produto 
					...
					And To_Date ( '01/' || LPad ( To_Char ( C_Conest . Cd_Mes ) , 2 , '0' ) || '/' || To_Char ( C_Conest . Cd_Ano ) , 'DD/MM/YYYY' ) Between TRUNC ( &Dt_Inicial ) 
					And Last_Day ( &Dt_Final )    
					AND PRODUTO.CD_ESPECIE In ( '2' ,'1' ,'6' )    
			Group By DECODE ( : P_PROD_MESTRE , 'S' , PRODUTO . CD_PRODUTO_TEM , PRODUTO . CD_PRODUTO ) , C_Conest . Cd_Produto ) consumo , dbamv . produto , dbamv . empresa_produto 
				Where 
					consumo . qt_consumo > 0 
					And consumo . cd_produto = produto . cd_produto And Empresa_Produto . cd_produto = produto . cd_produto And Empresa_Produto . Cd_Multi_Empresa = 1
					And Produto . Sn_Mestre = Nvl ( : P_PROD_MESTRE , 'N' ) 
